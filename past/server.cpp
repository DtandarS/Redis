...
//  Appends to the back of the buffer array
static void buf_append(std::vector<uint8_t> &buf, const uint8_t *data, size_t len)
{
  buf.insert(buf.end(), data, data + len);
}

//  Remnoves from buffer front
...
{
  assert(conn->outgoing_meows.size() = 0);
  ssize_t redVal = write(conn->fd, conn->outgoing_meows[0], conn->outgoing_meows.size());
  if (redVal < 0)
  {
    conn->want_close = true;
    return;
